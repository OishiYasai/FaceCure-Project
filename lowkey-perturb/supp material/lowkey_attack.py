...
# Set device and handle CPU-only machines
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Using device: {device}")

# Modified to handle CPU-only loading
...
if __name__ == '__main__':
    ...
    model_roots = ['models/Backbone_IR_152_Arcface_Epoch_112.pth', 
    ...
    # Modified prepare_models call to use device-aware loading
    def prepare_models_modified(model_backbones, input_size, model_roots, 
                              kernel_size_gf, sigma_gf, combination, 
                              using_subspace, V_reduction_root, device):
        models = []
        for i, backbone in enumerate(model_backbones):
            if backbone == 'IR_152':
                model = IR_152([input_size[0], input_size[1]]).to(device)
            elif backbone == 'ResNet_152':
                model = ResNet_152([input_size[0], input_size[1]]).to(device)
            
            model = load_model_weights(model, model_roots[i], device)
            model.eval()
            models.append(model)
        
        return models, None, 512  # Modified to return simplified values

    models_attack, V_reduction, dim = prepare_models_modified(
        model_backbones, input_size, model_roots, kernel_size_gf,
        sigma_gf, combination, using_subspace, V_reduction_root, device
    )
    ...
    while idx < len(imgs):
        print(f"Processing images {idx} to {min(idx+batch_size, len(imgs))} of {len(imgs)}")
        ...
        print(f"Batch shape: {tensor_img.shape}")

        V_reduction = None
        dim = 512

        # Find direction vector
        dir_vec_extractor = get_ensemble(models=models_attack, sigma_gf=None, 
                                       kernel_size_gf=None, combination=False, 
                                       V_reduction=V_reduction, warp=False, 
                                       theta_warp=None)
